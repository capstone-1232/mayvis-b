        ...
        <div class="container my-3">
            ...
            <form action="{{ route('clients.updateClient', $client->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control mb-3 rounded-pill" id="first_name" name="first_name" value="{{ $client->first_name }}">
                </div>
                <x-input-error class="mt-2 productserr" :messages="$errors->get('first_name')" />

                <div class="form-group">
                    <label for="last_name">Last Name</label>
                    <input type="text" class="form-control mb-3 rounded-pill" id="last_name" name="last_name" value="{{ $client->last_name }}">
                </div>
                <x-input-error class="mt-2 productserr" :messages="$errors->get('last_name')" />

                <div class="form-group">
                    <label for="company_name">Company Name</label>
                    <input type="text" name="company_name" field="company_name" class="form-control mb-3 rounded-pill" autocomplete="off" value="{{ $client->company_name }}"></input>
                </div>
                <x-input-error class="mt-2 productserr" :messages="$errors->get('company_name')" />

                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="email" name="email" field="email" class="form-control mb-3 rounded-pill" autocomplete="off" value="{{ $client->email }}"></input>
                </div>
                <x-input-error class="mt-2 productserr" :messages="$errors->get('email')" />

                <div class="form-group">
                    <label for="phone_number">Phone Number</label>
                    <input type="tel" name="phone_number" field="phone_number" placeholder="Client Phone" class="form-control mb-3 rounded-pill" autocomplete="off" value="{{ $client->phone_number}}"></input>
                </div>
