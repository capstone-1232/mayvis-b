            ...
            <div class="bg-white p-4 rounded-5 shadow mt-4">
                <form action="{{ route('services.updateProduct', $product->id) }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <x-text-input class="form-control" id="product_notes" name="product_notes" value="{{ $product->product_notes }}"></x-text-input>
                    </div>
                    <x-input-error class="mt-2 productserr" :messages="$errors->get('product_notes')" />
